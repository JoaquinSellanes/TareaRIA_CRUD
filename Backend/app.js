...
let hospitales = [
    { id: 1, name: 'Hospital Central', direccion: 'Av. Principal 123' },
    { id: 2, name: 'Hospital Norte', direccion: 'Calle Secundaria 456' },
    { id: 3, name: 'Hospital Sur', direccion: 'Boulevard Principal 789' }
];
...
// Ruta GET para obtener la lista de hospitales
app.get('/hospitales', addRandomDelay, (req, res) => {
    res.json(hospitales);
});

// Ruta GET para obtener un hospital por ID
...
// Ruta POST para agregar un nuevo hospital
app.post('/hospitales', addRandomDelay, (req, res) => {
    const newHospital = req.body;
    newHospital.id = hospitales.length ? hospitales[hospitales.length - 1].id + 1 : 1; // Asigna un nuevo id
    hospitales.push(newHospital);
    res.status(201).json(newHospital); // Responde con el nuevo hospital creado
});
...
// Ruta DELETE para eliminar un hospital
